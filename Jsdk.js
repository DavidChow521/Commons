...
!(function (window, $, undefined) {
    ...
    function init() {
        ...
        var arry = [];
        for (var fn in _methods) {
            arry.push(fn);
        }

        //初始化函数
        arry.forEach(function (f, e) {
            if (_methods[f] && typeof _methods[f] == "function") {
                Jsdk[f] = function () {
                    return _methods[f].apply(this, arguments);
                }
            } else {
                Jsdk[f] = _methods[f];
            }
        ...
        //初始化属性
        for (var args in defaults) {
            Jsdk[args] = defaults[args];
        }
    ...
    String.prototype.format = function () {
        Jsdk.format.apply(this, arguments);
    }
